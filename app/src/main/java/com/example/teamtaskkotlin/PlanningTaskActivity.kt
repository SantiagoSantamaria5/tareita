...
class PlanningTaskActivity : AppCompatActivity() {
        ...
        btnSavePlanning.setOnClickListener {
            if (!txtYear.text.isNullOrEmpty() && !txtMonth.text.isNullOrEmpty() && !txtDay.text.isNullOrEmpty() && !txtHrs.text.isNullOrEmpty()){
                ...
                if(selection=="-- Select Task --"){
                    ...
                    //Insert
                    if(createPlanningTask(listTask[spTaskPlanning.selectedItemPosition].id,txtYear.text.toString(),txtMonth.text.toString(),txtDay.text.toString(),txtHrs.text.toString())){
                        Toast.makeText(this,"Planning created", Toast.LENGTH_SHORT).show()
                        loadPlanningTask()
                    }
                    else{
                        Toast.makeText(this,"Error, can't create planning task", Toast.LENGTH_SHORT).show()
                    }
                }
            }
            else{
    ...
    fun selectAllPlanningTask():List<Task_Planning>{
        ...
        try{
            ...
            //Select
            val cursor = db.rawQuery("SELECT T.ID,T.NAME,T.STATUS,MG.RESPONSABLE,TP.YEAR,TP.MONTH,TP.HOURS FROM TASK T JOIN TASK_MANAGER MG ON MG.TASK_ID = T.ID AND T.STATUS NOT LIKE 'COMPLETED' JOIN TASK_PLANNING TP ON TP.TASK_ID = T.ID ORDER BY T.NAME"
                ,null)
            cursor.use {
                while (it.moveToNext()){
